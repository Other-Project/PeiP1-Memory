...
def calculeLongueurCote(tailleMax, nbCotes):
	"""Calcule la longueur des cotés d'un polynome régulier en
 	utilisant les propriétés du cercles circonscrit (https://en.wikipedia.org/wiki/Regular_polygon#Circumradius)
	et de l'apothème (https://fr.wikipedia.org/wiki/Apoth%C3%A8me)"""
	a = math.pi / nbCotes
	if nbCotes % 2:  # nb pair de cotés
	 cote = tailleMax * 2 * math.sin(a) / (1 + math.cos(a))
	else:          # nb impaire de cotés
...
def polygone(x, y, taille, couleur, t, nbCotes, resetHeading=True, fill=True):
	"""Dessine aux coordonnées (x,y) et avec la tortue t, un polygone à nbCotes, de taille et de couleur définies"""
...
def etoile(x, y, longueur, c, t):
	...
	polygone(x + taille, y + 2 * longueur / 3, taille, c, t, 3, False)  # Deuxième triangle
